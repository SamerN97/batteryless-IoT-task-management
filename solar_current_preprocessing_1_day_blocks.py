...
# Furthermore, it splits the data into 24h blocks, randomly shuffles these blocks, and then splits the shuffled data into a training set (80%) and an inference/validation set (20%). 
...
# 1. Truncate the array slightly so it is perfectly divisible by 8640
...
# 2. Reshape into a 2D array: (number_of_days, 8640)
# Now, each row represents exactly one 24-hour period.
...
print(f"Proceeding with {num_valid_chunks} perfectly stitchable 24h blocks.")

# # 3. Shuffle the 24h chunks randomly
...
print(f"Total 24h blocks: {num_chunks}")
...
plt.title("Shuffled 24h Blocks (80% Training Data)")
plt.show()
...
df2 = pd.DataFrame({"Shuffled Solar Training Data": train_data})
df2.to_csv("shuffled_solar_training_dataset.csv", index=False)
df3 = pd.DataFrame({"Shuffled Solar Validation Data": val_data})
df3.to_csv("shuffled_solar_validation_dataset.csv", index=False)
