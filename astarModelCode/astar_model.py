...
V_supply = 3.3
Vt_min = 1.8
max_rate_per_hour = 3600 / ti # max rate per hour that can be used for the curent simulation interval time
rate_per_hour = max_rate_per_hour
execTimer = max_rate_per_hour / rate_per_hour
hysteresis = 0.05
exec_trigger_time = 1
V_last_execution = V_to # Initialize memory of previous voltage
...
def get_adaptation_rate(Vnew, Vold, RateOld):

    if RateOld == 0:
        RateOld = 1

    if Vnew < (V_optimal - hysteresis):
        if Vnew > Vold:
            rate_per_hour = min((RateOld + 1), max_rate_per_hour)
        else:
            rate_per_hour = max((RateOld / 2), 1)
    elif Vnew > (V_optimal + hysteresis):
        if Vnew < Vold:
            rate_per_hour = max((RateOld - 1), 1)
        else:
            rate_per_hour = min((RateOld * 2), max_rate_per_hour)
    else:
        rate_per_hour = RateOld
    return rate_per_hour


def update_global_execution_timer(rate):
    if rate <= 0:
        return 120 # Wait max time if rate is 0
    # math.ceil ensures we wait at least 1 step
    return math.ceil(max_rate_per_hour / rate)
    ...
    if off == False:
        if Vt <= Vt_min: # Should be smaller or equal 
            ...
            if n < exec_trigger_time: 
                ...
                rate_per_hour = get_adaptation_rate(Vt, V_last_execution, rate_per_hour)
                V_last_execution = Vt
                execTimer = update_global_execution_timer(rate_per_hour)
                exec_trigger_time = n + execTimer
                Vt = capacitor_voltage(It, rSHT, tSHT, c, Vt)
        ...
        if Vt < V_to:
            ...
            off = False
            # We make sure the rate is put to 0 right after failure
            rate_per_hour = 1
            execTimer = update_global_execution_timer(rate_per_hour)
            exec_trigger_time = n + execTimer
            # This if statement will never be run because of the exec timer reset after failure
            if n == exec_trigger_time:
                rate_per_hour = get_adaptation_rate(Vt, V_last_execution, rate_per_hour)
                V_last_execution = Vt
                execTimer = update_global_execution_timer(rate_per_hour)
...
write_to_csv("astarModelData/voltageList" + additional_text + ".csv", capVoltageList)
write_to_csv("astarModelData/currentList" + additional_text + ".csv", finalHarvestingCurrentList)
write_to_csv("astarModelData/actionList" + additional_text + ".csv", actionList)
write_to_csv("astarModelData/successList" + additional_text + ".csv", successList)
write_to_csv("astarModelData/deviceStateList" + additional_text + ".csv", deviceStateList)
write_to_csv("astarModelData/tCycleList" + additional_text + ".csv", tCycleList)
write_to_csv("astarModelData/iCycleList" + additional_text + ".csv", iCycleList)
print(action_ctr)
print(success_ctr)
